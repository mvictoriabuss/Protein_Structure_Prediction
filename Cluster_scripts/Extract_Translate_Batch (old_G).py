...
def translate(seq, name) -> str:
    ...
    """
    #!! this function is applied after the n_count_translate function   
    table = {
    ...
    if len(seq)%3 == 0:
        for i in range(0, len(seq), 3):
            codon = seq[i:i + 3]
            #If there are no Ns in the nucleotide sequence --> everything is ok! Normal translation 
            if 'N' not in codon: 
                protein+= table[codon]
            #If 1+Ns are found, the triplette is translated as Glycine 
            else:
                protein+='G'
    else:
...
#This function is the first one applied to the Nucleotide sequence --> defines if (and how many) Ns are in the sequence and then applies the translation table
def n_count_translate(dna, name): 
    dna_len = len(dna)
    n_count = dna.count('N')
    calculate_percentage_n = (n_count/dna_len) * 100
    percentage_n = round(calculate_percentage_n, 1)
    
    #If there are Ns the script translates and says how many Ns
    if percentage_n != 0.0:
        print(f"{name} has {percentage_n} % Ns found in the sequence -> but translating anyway with Glycines")
        return translate(dna, name)
    
    #if there are no Ns, the translate(dna, name) function is applied as normal 
    else:
        return translate(dna, name)
...
def run():
    ...
    for t in targets:
        nuc_seq = ORFs[t][:].seq
        aa_seq = f'>{t}\n' + n_count_translate(nuc_seq, t)[:-1].strip()
